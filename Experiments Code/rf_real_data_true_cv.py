...
save_string = f"results/{dataset}_rfqr_experiment_true_cv_{agg}_randomizedconformal{randomized_conformal}_log{log_response}_recordCoverage{record_coverage}_{date_today}.pkl"
...
for train_index, test_index in kf.split(X):
    ...
    for params in ParameterSampler(param_dist, cv_iters, random_state=42):
        ...
        if model.uacqrp_average_length_test<best_score_uacqrp:
            best_params_dict['UACQR-P'] = params.copy()
            best_score_uacqrp = model.uacqrp_average_length_test

        if model.uacqrs_average_length_test<best_score_uacqrs:
            best_params_dict['UACQR-S'] = params.copy()
            best_score_uacqrs = model.uacqrs_average_length_test
        
        if model.cqr_average_length_test<best_score_cqr:
            best_params_dict['CQR'] = params.copy()
            best_score_cqr = model.cqr_average_length_test

        if model.cqrr_average_length_test<best_score_cqrr:
            best_params_dict['CQR-r'] = params.copy()
            best_score_cqrr = model.cqrr_average_length_test
        i+=1
    ...
    for method in ['UACQR-P','UACQR-S','CQR','CQR-r']:
        ...
        if record_coverage:
            ...
            df.loc[t,method] = model.evaluate(X_test_fold, y_test_fold)["average_length_test"][method]
    t+=1
